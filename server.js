...
var engine = require('./improvements.js');
...
io.on('connection', function (socket) {
  ...
  socket.on('guess', function (guess) {
    ...
    /* see if the guess is correct.  If so, send system message, set new word, and change drawer to the winner  */
    if (gameEngine.isGuessCorrect(guess)) {

      // send message about winner and make winner the drawer
      io.emit('gameMsg', nickName + ' ...the winner!');
  ...
  /* when a clear event is sent from the drawer, clear list of positions drawn and send clear to connected users. */
